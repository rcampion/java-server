 ...
 *
 * @see com.rkc.zds.AppMessageParam.spring.wiki.utils.WikiMessageParam
 */
public class AppMessage {

	private final String key;
	private AppMessageParam[] params = null;
	 ...
	 */
	public AppMessage(String key, String param1) {
		this.key = key;
		this.params = new AppMessageParam[1];
		params[0] = new AppMessageParam(this.escapeHtml4(param1));
	}
	 ...
	 *  HTML escaped to prevent erorrs in display.
	 */
	public AppMessage(String key, String param1, String param2) {
		this.key = key;
		this.params = new AppMessageParam[2];
		params[0] = new AppMessageParam(this.escapeHtml4(param1));
		params[1] = new AppMessageParam(this.escapeHtml4(param2));
	}
	 ...
	 */
	public AppMessage(String key, String[] params) {
		this.key = key;
		if (params != null) {
			this.params = new AppMessageParam[params.length];
			for (int i = 0; i < params.length; i++) {
				this.params[i] = new AppMessageParam(this.escapeHtml4(params[i]));
			}
		}
	}
	 ...
	 *  automatically HTML escaped to prevent erorrs in display.
	 */
	public AppMessage(String key, List<String> paramList) {
		this.key = key;
		if (paramList != null && !paramList.isEmpty()) {
			this.params = new AppMessageParam[paramList.size()];
			int i = 0;
			for (String param : paramList) {
				this.params[i++] = new AppMessageParam(this.escapeHtml4(param));
			}
		}
	}
	...
	public void addParam(String param) {
		AppMessageParam wikiMessageParam = new AppMessageParam(this.escapeHtml4(param));
	 ...
	 *
	 * @param wikiLinkParam A string that will be processed with the application:link tag
	 *  when this wiki message is rendered.
	 */
	public void addWikiLinkParam(String wikiLinkParam) {
		AppMessageParam wikiMessageParam = new AppMessageParam(wikiLinkParam, true);
		this.addParam(wikiMessageParam);
	}
	 ...
	 * @param wikiLinkParam A string that will be processed with the application:link tag
	 *  when this wiki message is rendered.
	 * @param wikiLinkParamText A string that will be used as text with the application:link
	 *  tag when this wiki message is rendered.
	 */
	public void addWikiLinkParam(String wikiLinkParam, String wikiLinkParamText) {
		AppMessageParam wikiMessageParam = new AppMessageParam(wikiLinkParam, wikiLinkParamText);
		this.addParam(wikiMessageParam);
	}
	 ...
	 */
	private void addParam(AppMessageParam wikiMessageParam) {
		AppMessageParam[] newParams = new AppMessageParam[this.getParamsLength() + 1];
		int i = 0;
		if (this.params != null) {
			for (AppMessageParam param : this.params) {
				newParams[i++] = param;
			}
		}
	 ...
	 * @return The array of parameter objects associated with this message.
	 */
	public AppMessageParam[] getParams() {
		return this.params;
	}
	...
	public void setParamsWithoutEscaping(String[] params) {
		this.params = new AppMessageParam[params.length];
		for (int i = 0; i < params.length; i++) {
			this.params[i] = new AppMessageParam(params[i]);
		}
	}
	...
	public void replaceParameter(int index, String parameter) {
		...
		this.params[index] = new AppMessageParam(this.escapeHtml4(parameter));
	}
